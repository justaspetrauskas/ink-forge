...
export function PromptEditor(): ReactElement {
  const [prompt, setPrompt] = useState("");

  const remainingCharacters = useMemo(() => MAX_PROMPT_LENGTH - prompt.length, [prompt.length]);

  return (
    <section className="rounded-[16px] border border-[var(--border-subtle)] bg-[var(--bg-main)] p-6 shadow-[var(--shadow-md)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-[24px] font-semibold tracking-[-0.02em] text-[var(--text-primary)]">Prompt Editor</h2>
          <p className="mt-1 text-sm text-[var(--text-secondary)]">Describe concept, placement, style, and mood with clear details.</p>
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {suggestedPrompts.map((item) => (
          <button
            ...
            className="rounded-full border border-[var(--border-medium)] bg-[var(--bg-surface)] px-3 py-1 text-xs text-[var(--text-secondary)] transition-colors duration-150 hover:bg-[var(--bg-raised)]"
          >
            {item}
          </button>
        ))}
